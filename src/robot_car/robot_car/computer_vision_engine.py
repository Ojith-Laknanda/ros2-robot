...
class ComputerVisionEngine(Node):
    def __init__(self, name):
        ...
        self.face_recognition_service = self.create_service(
            FaceRecognition,
            'face_recognition',
            self.face_recognition_callback,
        )

        self.describe_environment_service = self.create_service(
            DescribeEnvironment,
            'describe_environment',
            self.describe_environment_callback,
        )

        self.get_distance_closest_object_service = self.create_service(
            GetDistanceClosestObject,
        ...
        self.ojith_image = face_recognition.load_image_file("/home/ojith/ojith.jpg")
        self.ojith_encoding = face_recognition.face_encodings(self.ojith_image)[0]
    ...
    def get_distance_closest_object_callback(self, request, response):
        ...
        estimated_distance_cm = int(
            json.loads(self.get_claude_response(request.image_path, prompt))["estimated_distance_cm"]
        )
    ...
    def face_recognition_callback(self, request, response):
        ...
        response.identity = "Ojith" if results[0] else "unknown"

        self.get_logger().info(f"Identity: {response.identity}")
