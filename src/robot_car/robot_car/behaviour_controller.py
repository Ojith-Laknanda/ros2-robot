...
#             if self.Ojith_detected:
#                 self.get_logger().info('I am so happy! I found Ojith! :)')
#                 self.celebrate()
#             else:
#                 self.get_logger().info('wander!')
#                 self.wander(frame_path)
...
#         # sound alarm
#         msg_audio = PlayAudio()
#         msg_audio.audio_path = "/home/ojith/alarm.mp3"
#         self.publisher_voice.publish(msg_audio)

#         # display image
#         msg_display = DisplayImage()
#         msg_display.image_path = "/home/ojith/respond_to_intrussion.png"
        
#         self.get_logger().info(f"Publishing image: {msg_display.image_path}")
#         self.publisher_display.publish(msg_display)
...
#         if result.person_in_image and result.identity == "Ojith":
#             self.Ojith_detected = True
#         else:
#             self.Ojith_detected = False
...
#         msg_display = DisplayImage()
#         if self.mode == "explore":
#             msg_display.image_path = '/home/ojith/wander_explore.png'
#         elif self.mode == "patrol":
#             msg_display.image_path = '/home/ojith/wander_patrol.png'
#         elif self.mode == "hide_and_seek":
#             msg_display.image_path = '/home/ojith/wander_hide_and_seek.png'
#         self.publisher_display.publish(msg_display)
...
from computer_vision_interface.srv import (
    DescribeEnvironment,
    FaceRecognition,
)
from teleop_interface.msg import Teleop
from ui_interface.msg import DisplayImage, PlayAudio
...
class BehaviourController(Node):
    ...
    def __init__(self, name):
        ...
        # Publishers
        self.publisher_teleop = self.create_publisher(
            Teleop,
            "teleop_robot_cmds",
            10,
        )

        self.publisher_voice = self.create_publisher(
            PlayAudio,
            "voice_cmds",
            10,
        )

        self.publisher_display = self.create_publisher(
            DisplayImage,
            "display_cmds",
            10,
        )
        ...
        # Service clients
        self.face_recognition_client = self.create_client(
            FaceRecognition,
            "face_recognition",
        )

        self.describe_environment_client = self.create_client(
            DescribeEnvironment,
            "describe_environment",
        )

        # Wait for required services
        while not self.face_recognition_client.wait_for_service(
            timeout_sec=1.0
        ):
            self.get_logger().info(
                "Waiting for face recognition service..."
            )

        while not self.describe_environment_client.wait_for_service(
            timeout_sec=1.0
        ):
            self.get_logger().info(
                "Waiting for describe environment service..."
            )
        ...
        if not self.video_cap.isOpened():
            self.get_logger().error(
                "Could not open camera /dev/video0"
            )

        # Prevent multiple face requests at the same time
        self.face_request_pending = False

        # Latest camera image
        self.latest_frame_path = str(
            self.home_dir / "tmp.png"
        )

        # Anthropic client
        self.ai_client = anthropic.Anthropic()

        # Run behaviour every second
        ...
        self.get_logger().info(
            f"Behaviour controller started in {self.mode} mode"
        )
            ...
            if param.name == "mode":
                if param.value not in self.valid_modes:
                    return SetParametersResult(
                        successful=False,
                        reason=(
                            "Mode must be explore, patrol, "
                            "or hide_and_seek"
                        ),
                    )

                self.get_logger().info(
                    f"Mode changed to: {param.value}"
                )
    ...
    def capture_frame(self):
        ...
        # Remove older buffered frames
        start_time = time.time()

        while time.time() - start_time < 0.3:
            new_ret, new_frame = self.video_cap.read()
        ...
        if not saved:
            self.get_logger().error(
                f"Failed to save: {self.latest_frame_path}"
            )
            return None
    ...
    def behaviour_callback(self):
        ...
        if self.mode == "explore":
            ...
            self.request_face_recognition(
                frame_path,
                self.face_recognition_patrol_callback,
            )

        elif self.mode == "hide_and_seek":
            self.request_face_recognition(
                frame_path,
                self.face_recognition_hide_and_seek_callback,
            )

    def request_face_recognition(
        self,
        frame_path,
        callback,
    ):
        # Do not send another request while one is running
        if self.face_request_pending:
            return

        self.face_request_pending = True
        self.latest_frame_path = frame_path

        request = FaceRecognition.Request()
        request.image_path = frame_path

        future = self.face_recognition_client.call_async(
            request
        )

        future.add_done_callback(callback)

    def face_recognition_patrol_callback(self, future):
        self.face_request_pending = False
        ...
        try:
            ...
            self.get_logger().info(
                f"Face result: "
                f"person={result.person_in_image}, "
                f"identity={result.identity}"
            )

            # Ojith is recognized
            if (
                result.person_in_image
                and result.identity.strip().lower() == "ojith"
            ):
                self.get_logger().info(
                    "Congratulations! Ojith identified!"
                )

                self.celebrate()

            # Another person is detected
            elif result.person_in_image:
            ...
            # No person detected
                ...
                self.get_logger().info(
                    "No person detected. Wandering."
                )

                self.wander(self.latest_frame_path)
    ...
    def face_recognition_hide_and_seek_callback(
        self,
        future,
    ):
        self.face_request_pending = False
        ...
        try:
            ...
            if (
                ...
                self.get_logger().info(
                    "Ojith not found. Continuing search."
                )
    ...
    def celebrate(self):
        # Stop motors
        motor_message = Teleop()
        motor_message.direction = "stop"
        motor_message.duration = 100
        motor_message.speed = 255

        self.publisher_teleop.publish(
            motor_message
        )

        # Play celebration sound
        audio_message = PlayAudio()
        audio_message.audio_path = str(
            self.home_dir / "yay.mp3"
        )

        self.publisher_voice.publish(
            audio_message
        )

        # Display celebration image
        display_message = DisplayImage()
        display_message.image_path = str(
            self.home_dir / "celebrate.png"
        )

        self.publisher_display.publish(
            display_message
        )

        self.get_logger().info(
            f"Playing: {audio_message.audio_path}"
        )

        self.get_logger().info(
            f"Displaying: {display_message.image_path}"
        )
    ...
    def respond_to_intrusion(self):
        ...
        # Display intrusion image
        display_message = DisplayImage()
        display_message.image_path = str(
            self.home_dir
            / "respond_to_intrussion.png"
        )
    ...
    def describe_environment(self, image_path):
        request = DescribeEnvironment.Request()
        request.image_path = image_path

        future = (
            self.describe_environment_client.call_async(
                request
            )
        )
    ...
    def describe_environment_callback(
        self,
        future,
    ):
        try:
            ...
            self.get_logger().info(
                f"Environment: "
                f"{result.image_description}"
            )

        except Exception as error:
            self.get_logger().error(
                f"Environment description failed: {error}"
            )

    def get_distance_closest_object(
        self,
        image_path,
    ):
        prompt = """
...
Estimate the distance in centimeters to the closest object
directly ahead.

If the image is blocked, unreadable, too dark, or too close,
set estimated_distance_cm to 0.
...
"""
        ...
        try:
            ...
            return int(
                result_json["estimated_distance_cm"]
            )

        except Exception as error:
            self.get_logger().error(
                f"Distance estimation failed: {error}"
            )

            # Zero causes the robot to move backward safely
            return 0
    ...
    def wander(self, image_path):
        ...
        if self.mode == "explore":
            display_message.image_path = str(
                self.home_dir / "wander_explore.png"
            )

        elif self.mode == "patrol":
            display_message.image_path = str(
                self.home_dir / "wander_patrol.png"
            )

        elif self.mode == "hide_and_seek":
            display_message.image_path = str(
                self.home_dir
                / "wander_hide_and_seek.png"
            )
        ...
        distance = self.get_distance_closest_object(
            image_path
        )

        self.get_logger().info(
            f"Distance to closest object: {distance} cm"
        )
        ...
        if distance > 15:
            ...
            if random_value < 0.5:
                motor_message.direction = "forward"

            elif random_value < 0.75:
                motor_message.direction = "left"

            else:
                motor_message.direction = "right"

        else:
            motor_message.direction = "backward"

        motor_message.duration = (
            300 + int(random.random() * 500)
        )
...
def main(args=None):
    ...
    node = BehaviourController(
        "behaviour_controller_node"
    )
